...
import {styles, fieldForceListStyleValues} from '../styles';
var {
  ListView,
  Platform,
  TouchableHighlight,
...
export const FieldForceListView = React.createClass({
  ...
  getInitialState() {
    ...
    } else if( this.props.rowChangedFunc ) {
       ds = new ListView.DataSource({
        rowHasChanged: this.props.rowChangedFunc,
      });
  ...
  componentDidMount() {
    ...
    if(this.props.setListState){
      this.props.setListState(this._updateRows)
    }
    this.setDataSourceRows(this.props.data);
    this._postRefresh();
  ...
  _onRefresh(options = {}) {
    ...
    if(this.props.refreshCallback){
      this.props.refreshCallback(this._postRefresh, options);
    } 
  },
  ...
  _onPaginate() {
    ...
    if(this.props.getPageCallback){
      this.props.getPageCallback(this._getPage() + 1, this._postPaginate, {})
    }
  ...
  componentWillReceiveProps(nextProps){
    if(nextProps.data !== this.props.data)
      this.setDataSourceRows(nextProps.data);
  },
  render() {
    return (
      <ListView
  ...
  defaultStyles: {
    separator: {
      height: 1,
      backgroundColor: fieldForceListStyleValues.separator.backgroundColor
    },
    refreshableView: {
      height: 50,
      backgroundColor: fieldForceListStyleValues.refreshableView.backgroundColor,
      justifyContent: 'center',
      alignItems: 'center',
    },
    actionsLabel: {
      fontSize: 20,
      color: fieldForceListStyleValues.actionsLabel.color
    },
    paginationView: {
      height: 44,
      justifyContent: 'center',
      alignItems: 'center',
      backgroundColor: fieldForceListStyleValues.paginationView.backgroundColor,
    },
    defaultView: {
      justifyContent: 'center',
      alignItems: 'center',
