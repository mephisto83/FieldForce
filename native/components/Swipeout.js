...
, _handlePanResponderGrant: function(e: Object, gestureState: Object) {
    ...
    this.refs.swipeoutContent.measureLayout(React.findNodeHandle(this),(ox, oy, width, height) => {
        this.setState({
        btnWidth: (width/5),
...
, _handlePanResponderMove: function(e: Object, gestureState: Object) {
    ...
    if (this.state.swiping) {
      ...
      if (posX < 0 && this.props.right){
           ...
           var value = Math.min(posX, 0);
           this.state.contentPos.setValue(value);
           this.setState({_contentPos: value})
      }
      else if (posX > 0 && this.props.left){
           // this.setState({ contentPos: Math.max(posX, 0) })
           var value = Math.max(posX, 0);
           this.state.contentPos.setValue(value);
           this.setState({_contentPos: value})
      }
    }
...
, _handlePanResponderEnd: function(e: Object, gestureState: Object) {
    var posX = gestureState.dx
    var contentPos = this.state._contentPos;
    ...
    var difference = (new Date()).getTime() - this.state.timeStart;
    var timeDiff = difference < 200
    if (timeDiff) {
    ...
    if (this.state.swiping) {
      if (openRight && contentPos < 0 && posX < 0) {
        // open swipeout right
        // this._tweenContent(this.state.contentPos, -btnsRightWidth)   
        this.setState({_contentPos: -btnsRightWidth, openedLeft: false, openedRight: true })
      } else if (openLeft && contentPos > 0 && posX > 0) {
        // open swipeout left
        //this._tweenContent(this.state.contentPos, btnsLeftWidth)
        this.setState({_contentPos: btnsLeftWidth, openedLeft: true, openedRight: false })
      }
      else {
        // close swipeout
        // this._tweenContent(this.state.contentPos, 0);
        this.setState({_contentPos: 0, openedLeft: false, openedRight: false })
      }
    }
...
, _rubberBandEasing: function(value, limit) {
    if (value < 0 && value < limit) return limit - Math.pow(limit - value, 0.35);
    else if (value > 0 && value > limit) return limit + Math.pow(value - limit, 0.35);
    return value;
  }
...
, _close: function() {
    ...
    // Animated.timing(this.state.contentPos,{toValue:0}, ).start();
    // this._tweenContent(this.state.contentPos, 0)  
    this.setState({
      _contentPos: 0,
      openedRight: false,
...
, render: function() {
    ...
    var posX =  self.state._contentPos; // self.getTweeningValue('contentPos')
